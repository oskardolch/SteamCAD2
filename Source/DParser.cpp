...
PDUnit CDUnitList::FindUnit(const char *psAbbrev)
{
  ...
  {
    pUnit = &m_pData[i++];
    bFound = ((strcmp(pUnit->sAbbrev, psAbbrev) == 0) || (strcmp(pUnit->sAbbrev2, psAbbrev) == 0));
  }
...
{
  return (*psNumber == '+') || (*psNumber == '-') || (*psNumber == '*') || (*psNumber == '/');
}
...
{
  bool bRes = ((*psNumber >= '0') && (*psNumber <= '9')) || (*psNumber == '.') ||
    (*psNumber == ',') || IsOperChar(psNumber) || (IsPI(psNumber)) || (*psNumber == ' ');
  return bRes;
}
